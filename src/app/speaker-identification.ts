...
export class SpeakerIdentification {
  ...
  public GetSpeaker(counter: number): Speaker {
    ...
    if (this.speakerMap[counter]) {
      console.log('GetSpeaker found ', {
        ...this.speakerMap[counter],
        text: this.paragraphs[counter],
      });
      if (
        this.processedUnitlCounter <=
        counter + SpeakerIdentification.autoCallStep
      ) {
        console.log(
          'TriggerSpeakerIdentification call auto',
          this.processedUnitlCounter,
        );
  ...
  private async TriggerSpeakerIdentification(
    counter: number,
  ): Promise<boolean> {
    if (!this.GetEnabled()) {
      throw new Error('SpeakerIdentification is disabled');
    ...
    const paragraphsBefore = this.paragraphs.slice(
      startCounter - SpeakerIdentification.textsBeforeStep,
      startCounter,
    );
    // [startCounter, endCounter)
    const processingParagraphs = this.paragraphs.slice(
      startCounter,
      endCounter,
    );
    // [endCounter, endCounter + 5)
    const paragraphsAfter = this.paragraphs.slice(
      endCounter,
      endCounter + SpeakerIdentification.textsAfterStep,
    );
    try {
